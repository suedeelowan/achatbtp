...
if ( ! function_exists( 'unicase_setup' ) ) :
	...
	function unicase_setup() {
		...
		register_nav_menus( apply_filters( 'unicase_register_nav_menus', array(
			...
			'hand-held'		=> esc_html__( 'Handheld Menu', 'unicase' )
		) ) );
...
if ( ! function_exists( 'unicase_scripts' ) ) :
	...
	function unicase_scripts() {
		...
		$unicase_option = array(
			...
			'live_search_limit'		=> apply_filters( 'unicase_live_search_limit', 10 ),
		...
		if( apply_filters( 'unicase_load_all_minifed_js', true ) ) {
			...
			if( apply_filters( 'unicase_enable_sticky_header', false ) ) {
				wp_enqueue_script( 'waypoints-js', get_template_directory_uri() . '/assets/js/waypoints.min.js', array( 'jquery' ), $unicase_version, true );
				wp_enqueue_script( 'waypoints-sticky-js', get_template_directory_uri() . '/assets/js/waypoints-sticky.min.js', array( 'jquery', 'waypoints-js' ), $unicase_version, true );
			}
	...
	function unicase_register_required_plugins() {
		...
		$plugins = array(
	        ...
	        array(
	            ...
	            'source'				=> 'https://transvelo.github.io/unicase/assets/plugins/unicase-extensions.zip',
	            'required'				=> false,
	            'version'				=> $unicase_version,
	            'force_activation'		=> false,
	            'force_deactivation'	=> false,
	            'external_url'			=> '',
	        ),
		...
		$config = apply_filters( 'unicase_tgmpa_config', array(
			'domain'       		=> 'unicase',
			'default_path' 		=> '',
