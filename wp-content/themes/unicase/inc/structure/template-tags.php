...
if ( ! function_exists( 'unicase_blog_carousel' ) ) {
	 ...
	 * @return  void
	 */
	function unicase_blog_carousel( $args = array(), $limit = 12 , $orderby = 'title', $order = 'ASC', $disable_touch_drag = FALSE ) {
		...
		$query_args = apply_filters( 'unicase_blog_carousel_query_args', $query_args );
	    
		wp_enqueue_script( 'owl-carousel-js', get_template_directory_uri() . '/assets/js/owl.carousel.min.js', array( 'jquery' ), '1.10.2', true );

		unicase_get_template( 'sections/blog-carousel.php', array( 'query_args' => $query_args, 'title' => $args['title'], 'disable_touch_drag' => $args['disable_touch_drag'] ) );
	}
}
		...
		if( is_woocommerce_activated() ) {
			...
			if( ! empty( $brand_taxonomy ) ) {
				...
				if( ! is_wp_error( $terms ) && !empty( $terms ) ) {
					...
					unicase_get_template( 'sections/brands-carousel.php', array( 'terms' => $terms, 'title' => $title, 'disable_touch_drag' => $disable_touch_drag, 'brand' => $brand ) );
				}
			}
		}
	}
