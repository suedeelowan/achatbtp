...
class Convert {
	...
	public function plugin_import_description( $counter, $pluginName ) {
		return Helpers::wp_kses_i18n( sprintf( __( 'We found you have %1$s categories you created from <strong>%2$s</strong> plugin. Would you like to import it to <strong>FileBird</strong>?', 'filebird' ), $counter, $pluginName ) );
	}
...
<div class="njt notice notice-warning <?php echo esc_attr( $site['site'] ); ?> is-dismissible">
    ...
    <p>
		...
		echo Helpers::wp_kses_i18n( sprintf( __( 'We found you have %1$s categories you created from <strong>%2$s</strong> plugin. Would you like to import it to <strong>FileBird</strong>?', 'filebird' ), $c, $site['title'] ) );
			?>
    </p>
    ...
    <button class="button njt_fb_no_thanks_btn"
        data-site="<?php echo esc_attr( $site['site'] ); ?>"><?php esc_html_e( 'No, thanks', 'filebird' ); ?></button>
    </p>
</div>
<?php endforeach; ?>
<?php
	...
	public function ajaxImportAfterInserting( $request ) {
		...
		$mess = Helpers::wp_kses_i18n( sprintf( __( 'Congratulations! We imported successfully %d folders into <strong>FileBird.</strong>', 'filebird' ), $count ) );
		wp_send_json_success(
			array(
				'mess' => $mess,
			)
	...
	public function getAttOfFolder( $site, $folder ) {
		...
		if ( $site == 'enhanced' ) {
			...
			$att = $wpdb->get_col( $wpdb->prepare( "SELECT attachment FROM {$wpdb->prefix}realmedialibrary_posts WHERE fid = %d", $folder->id ) );
		} elseif ( $site == 'happyfiles' ) {
			$att = $wpdb->get_col( $wpdb->prepare( "SELECT object_id FROM $wpdb->term_relationships WHERE term_taxonomy_id = %d", $folder->term_taxonomy_id ) );
		} elseif ( $site == 'premio' ) {
			$att = $wpdb->get_col( $wpdb->prepare( "SELECT object_id FROM $wpdb->term_relationships WHERE term_taxonomy_id = %d", $folder->term_taxonomy_id ) );
