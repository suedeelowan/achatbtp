...
class SizeMeta {
    ...
    public function manage_media_columns( $posts_columns ) {
		$posts_columns[ $this->meta_key ] = __( 'File Size', 'filebird' );
    ...
    public function get_size( $post_id ) {
	...
	public function manage_media_custom_column( $column_name, $post_id ) {
		if ( $this->meta_key === $column_name ) {
            if ( ! $this->get_size( $post_id ) ) {
                $file_size = $this->generate_meta_size( $post_id );
            } else {
                $file_size = $this->get_size( $post_id );
            }
